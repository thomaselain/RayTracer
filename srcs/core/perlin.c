...
/*   Updated: 2017/04/19 20:02:43 by svassal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		*get_permu(void)
{
	return ((int[512]){151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194,
		...
		47, 16, 58, 17, 182, 189, 28, 42, 23, 183, 170, 213, 119, 248, 152,
...
static void		sub_perlin(int p[512], int n[9], float f[3], float d[3])
{
	P_U = FADE(f[0]);
	P_V = FADE(f[1]);
	P_W = FADE(f[2]);
...
float			perlin(float x, float y, float z)
{
	...
	y -= floor(y);
	z -= floor(z);
	sub_perlin(perlin, n, (float[3]){x, y, z}, d);
	return (LERP(P_W, LERP(P_V, LERP(P_U, grad(perlin[P_AA], x, y, z),
						grad(perlin[P_BA], x - 1, y, z)),
					LERP(P_U, grad(perlin[P_AB], x, y - 1, z),
